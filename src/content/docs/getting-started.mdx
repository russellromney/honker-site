...
Requires Python 3.11+. The wheel bundles the Rust cdylib so you don't need a separate extension download. macOS (arm64, x86_64), Linux (x86_64, aarch64), and Windows (x86_64) wheels are published.
  </TabItem>
  <TabItem label="Node">
```bash
...
Requires Node 20+. The `.node` native binary is built per-platform by the release workflow, so `honker.open()` needs no separate extension download on supported targets (macOS arm64/x86_64, Linux x86_64/aarch64). To load Honker onto your own better-sqlite3 or Drizzle connection instead, use the loadable extension — it installs alongside as an optional dependency and `require('@russellthehippo/honker-node/extension').extensionPath()` returns its path. No Windows or musl target yet.
  </TabItem>
  <TabItem label="Rust">
```toml
# Cargo.toml
...
Requires Go 1.25+. The Go binding calls into the extension via `mattn/go-sqlite3`, so you also need the compiled `libhonker_ext.{dylib,so,dll}` on disk — Go modules can't ship a binary. Download it from a [release](https://github.com/russellromney/honker/releases) or build it yourself (see the SQLite extension tab), then point `HONKER_EXTENSION_PATH` at it and call `honker.ExtensionPath()`.
  </TabItem>
  <TabItem label="Ruby">
```ruby
# Gemfile
...
Requires Bun 1.3+. Uses `bun:sqlite`. The extension installs automatically as an optional dependency; `extensionPath()` returns its location. macOS (arm64, x64) and Linux (x64, arm64, glibc) are prebuilt — no Windows or musl target yet.
  </TabItem>
...
Requires Elixir 1.17+ and Erlang/OTP 26+. Uses `exqlite` for the SQLite FFI and loads the extension from a configured path. Hex can't ship a binary, so download it from a [release](https://github.com/russellromney/honker/releases) or build it, then use `Honker.Extension.path!/0`.
  </TabItem>
  <TabItem label=".NET">
```bash
...
Requires Zig 0.15+, a C++17 compiler, and SQLite 3.9+ with extension loading. On macOS use Homebrew's `sqlite3` (Apple's system build omits `load_extension`). Link against the built `libhonker_cpp.a` static library and include `include/honker.hpp`.
  </TabItem>
  <TabItem label="SQLite extension">
Download `libhonker_ext` for your platform from a [release](https://github.com/russellromney/honker/releases) — macOS (arm64, x64), Linux (x64, arm64), and Windows (x64), with `SHA256SUMS` alongside.

Or build it. It's one crate:
...
Works with any SQLite 3.9+ client: `sqlite3` CLI, `better-sqlite3`, `rusqlite`, Go's `mattn/go-sqlite3`, Ruby's `sqlite3` gem, Elixir's `exqlite`, and so on.

Most bindings ship it for you and can tell you where it is — see [binding support](https://github.com/russellromney/honker/blob/main/BINDINGS.md). Keep the `libhonker_ext` name: SQLite derives the entrypoint from the filename, so a rename means passing `sqlite3_honkerext_init` explicitly.
  </TabItem>
</Tabs>
...
The worker loop wakes in the low-single-digit milliseconds on reference hardware when a new job lands, via a shared 1 ms `PRAGMA data_version` poll on the database. Exact latency varies by binding, OS, and hardware; see [Benchmarks](/reference/benchmarks/). No application-level polling. (Two experimental alternatives — kernel filesystem events and mmap'd WAL-index reads — are available behind opt-in flags; see [Watcher backends](/reference/watcher-backends/).)
...
for await (const job of q.claim('worker-1')) {
  try {
    ...
    // Scheduled retry with backoff. After maxAttempts (default 3),
    // the job moves to _honker_dead with last_error set.
...
- [Function tasks](/guides/tasks/) for Python decorators and explicit Java/Kotlin registries
