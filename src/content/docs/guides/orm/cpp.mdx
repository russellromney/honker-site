...
The C++ binding takes the extension path explicitly — it links `libhonker_cpp.a` but does not bundle the loadable extension. Download `honker-ext-<target>.tar.gz` from a [release](https://github.com/russellromney/honker/releases) and extract `libhonker_ext.{dylib,so}` from it, or build it with `cargo build --release -p honker-extension`. Keep the file name as-is: SQLite derives the extension entry point from it.
