...
Connect through a connection factory, not the `url` + `driver` overload. That overload never sees the `SQLiteConfig` properties, so extension loading is still off when the connection opens and `load_extension` fails with `not authorized`.
...
Database.connect({
    DriverManager.getConnection("jdbc:sqlite:app.db", props).also { conn ->
...
Then the transaction body is just SQL:
