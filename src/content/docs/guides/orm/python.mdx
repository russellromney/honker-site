...
`honker.load_extension()` finds the extension bundled in the wheel and runs the `enable_load_extension` toggle around the load, so the toggle can't be left on. `honker.extension_info()` returns the same path and its entrypoint if you'd rather load it yourself.
...
class TypedQueue(Generic[T]):
    ...
    def parse(self, raw: dict) -> T:
        """Worker-side: validate a decoded job payload back into the model."""
