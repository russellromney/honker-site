...
Honker's pub/sub is a direct analog of Postgres's `pg_notify` / `LISTEN`: fire-and-forget signals between processes that share a SQLite file. An Apple M1 Pro measured 2.9 ms p50 cross-process wake latency over 500 samples. No replay, no delivery guarantees. For that, use [Streams](/guides/streams/).
