...
# What's scheduled? Each row has cron, payload, priority, enabled, next_fire_at.
...
`pause` / `resume` are idempotent — calling pause on an already-paused schedule returns `false`. `update` with no field args is a no-op (returns `false`). All methods are available in every binding (Python, Node, Bun, Rust crate, Go, Ruby, Elixir, .NET, C++).
