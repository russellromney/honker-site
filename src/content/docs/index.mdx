...
honker adds Postgres-style `NOTIFY`/`LISTEN` semantics to SQLite, with a durable pub/sub, task queue, and event streams on the side, without client polling or a daemon/broker. Cross-process wake latency is ~2.9 ms p50 on an Apple M1 Pro (500 samples).
