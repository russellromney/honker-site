...
- **Try shm** if you're on a high-QPS WAL workload and the per-check cost of the 1 ms poll matters (rare). The cadence stays at 1 ms like polling, but each check drops from a ~3.5 µs `PRAGMA data_version` read to a memory load. The catch: it reads SQLite's WAL index format directly, so a future SQLite version that changes that format would break it.
...
Good:
- Cheaper checks. A memory load instead of a SQLite query — the per-check cost drops from ~3.5 µs to sub-microsecond (the poll cadence is still 1 ms).
...
These commands cover the bindings that compile the Rust watcher directly.
Bindings that load the SQLite extension use the capabilities of the extension
artifact they bundle or load; see [Binding support](/reference/bindings/).
