...
Honker watches for committed database changes and wakes consumers so they can re-read state. By default, one background thread per database asks SQLite "did anything change?" every millisecond. Several commits inside one polling interval can coalesce into one wake.

Two opt-in alternatives exist for advanced users. In the Rust-backed Python and Node packages, both are experimental and source-build-only. The JVM implements equivalent backends in Java and ships them in its artifact; configuration entry points vary in the other bindings. See [Binding support](/reference/bindings/) before selecting one outside the Python, Node, or Rust APIs shown here.
...
In the Rust watcher, the kernel and shm backends are gated behind Cargo features (`kernel-watcher`, `shm-fast-path`). The published Python wheel and Node native package build polling-only. To use the experimental Rust backends today, build from source with the feature on — see [Source builds](#source-builds) below. These Cargo flags do not control the JVM's Java implementations.

## At a glance

| backend | wake source | spurious wakes | delayed/coalesced wakes | journal mode | Python/Node prebuilt package |
|---|---|---|---|---|---|
| `polling` (default) | `PRAGMA data_version` every 1 ms | never | possible between polls | any | yes |
| `kernel` | OS file events on db file + parent dir + `-wal`/`-shm`/`-journal` | possible | possible (if the OS drops events) | any | no — source only |
| `shm` | mmap read of SQLite's WAL index every 1 ms | never | possible between polls (mmap inode-change → panic) | WAL only | no — source only |

## When to choose which

- **Stay on polling** unless you have a specific reason to switch. It does not depend on an OS event queue, and consumers re-read state after a wake; multiple commits between checks can still coalesce. It works in every journal mode and is the backend in the prebuilt Python and Node packages. The CPU cost — about 2.0 ms per second of busy work on the benchmark machine — is rounding error on any modern machine.
...
- **Try shm** if you're on a high-QPS WAL workload and the per-check cost of the 1 ms poll matters (rare). The cadence stays at 1 ms like polling, but each check drops from a ~2.0 µs `PRAGMA data_version` read to a memory load. The catch: it reads SQLite's WAL index format directly, so a future SQLite version that changes that format would break it.
...
One background thread per database. Every millisecond, it runs `PRAGMA data_version` on a read-only connection. That counter changes after commits by another connection, in any journal mode. Around 2.0 microseconds per check on the standalone Rust benchmark machine. One thread regardless of subscriber count.
...
Good:
- Cheaper checks. A memory load instead of a SQLite query — the per-check cost drops from ~2.0 µs to sub-microsecond (the poll cadence is still 1 ms).
- Same low cost under heavy load.
...
The examples and failure shapes in this section cover bindings that expose the
raw update-event stream directly. Other bindings surface the same watcher
failure through their queue, listener, or stream abstraction; check the
binding's language guide for its public API.
...
- **Node:** `await events.next()` rejects with a `'watcher died'` message. Older versions reject with `"receiving on a closed channel"` instead.

Not a silent hang. Not just a line in stderr you'll never read.

### Recovery pattern

The recovery snippets below are Python and Node examples, not a universal API
shape. In every binding, the invariant is the same: close the failed database
handle and open a new one after the database file has been replaced.
...
async function consume() {
  ...
  while (true) {
    try {
      ...
      // 'watcher died' on current versions; older versions say
      // 'closed channel' — which close() also rejects a pending wait with.
      if (
        !String(e.message).includes('watcher died') &&
        !String(e.message).includes('closed channel')
      ) throw e;
      db.close();
      db = honker.open('app.db');
    }
...
These commands cover the Rust wrapper and the Python/Node bindings that compile
the Rust watcher directly. The FFI bindings depend on the watcher capabilities
of the native artifact they load or link. The JVM is different again: its
watchers are implemented in Java and are not selected by Cargo feature flags.
See [Binding support](/reference/bindings/).
...
If the requested backend can't start at runtime (shm needs WAL plus an open connection; kernel needs the OS notify API), `honker.open()` raises right away. If you ask for a backend whose Cargo feature isn't compiled into your binary (the published-wheels case), the open call fails loudly too — it throws with an error naming the missing feature. No binding silently falls back to polling.
