...
language wrapper does not expose the nice API yet. "local" under Package
proof means the proof runs outside root CI (see below). For install and ORM
...
| Java `dev.honker:honker` | local | yes | yes | yes | yes | yes | poll, kernel, shm |
| Kotlin `dev.honker:honker-kotlin` | local | yes | yes | yes | yes | yes | poll, kernel, shm |
...
- Java and Kotlin are proven locally: Maven package tests plus a clean
  consumer proof outside the repo. No JVM/Kotlin jobs run in PR CI.
